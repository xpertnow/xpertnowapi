...
const getUserConsultationById = async (request, response) => {
  ...
  ON 
    vcm.other_user_id = expert.user_id AND expert.user_type = 2 
    LEFT JOIN  expert_earning_master em ON vcm.video_call_id = em.video_call_id
  WHERE 
...
const getEarningDetailsById = async (request, response) => {
    ...
    var Fetchpost =
      "SELECT em.expert_earning_id, em.user_id, em.expert_id, em.total_amount, em.expert_earning,  em.expert_type, em.gst_per, em.gst_amt, em.net_expert_earning, em.tds_per, em.tds_amt, em.tcs_per, em.tcs_amt, em.platform_fees, em.platform_fees_gst_amt, em.grand_total_expert_earning, em.type, um.name AS customer_name, umm.name AS expert_name, em.createtime  FROM expert_earning_master em JOIN user_master um ON em.user_id = um.user_id JOIN user_master umm ON em.expert_id = umm.user_id WHERE em.expert_earning_id = ? AND em.delete_flag = 0 AND um.delete_flag = 0 ORDER BY em.createtime DESC";
    try {
    ...
    try {
        ...
        connection.query(sql, [user_id], async (err, res) => {
            ...
            connection.query(update, [new_call_charge, user_id], async (err1, res1) => {
                ...
                if (res1.affectedRows == 0) {
                    return response.status(200).json({ success: false, msg: 'Request has not been accepted' });
                }
                return response.status(200).json({ success: true, msg: 'request accepted successfully' })
            })
...
const rejectCallChargeRequest = async (request, response) => {
    ...
    try {
        ...
        connection.query(sql, [user_id], async (err, res) => {
            ...
            // let new_call_charge = res[0].new_call_charge;
            const update = 'UPDATE user_master SET call_charge_status = 2 WHERE user_id = ? AND delete_flag = 0';
