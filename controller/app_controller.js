...
const {
  ...
  getCategoryName,
  getHomeExpertJob,
  getHomeExpertUserJob,
  getHomeUserJobCount,
  getCustomerJobFilter,
  getAvgRating,
  getExpertJobFilter,
  getJobMilestone,
  getJobWorkSpace,
  getHomeExpertCompletedJob,
  authenticateToken,
  getUserTotalWallet,
} = require("../shared functions/functions");
  ...
  try {
    ...
    connection.query(query, values, async (err, result) => {
      ...
      if (result.length > 0) {
        ...
        const filteredDetails = expertDetails.filter(
          (expert) => expert !== null
        );

        return response
          .status(200)
          .json({ success: true, expertDetails: filteredDetails });
      } else {
        return response
          .status(200)
          .json({ success: true, expertDetails: "NA" });
      }
    });
  } catch (error) {
    console.error("Error fetching expert details:", error);
...
const getExpertDetailsById = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag FROM user_master WHERE user_id = ? AND delete_flag=0 AND user_type=1";
  ...
  try {
    ...
    connection.query(chatSql, [user_id], async (chatErr, chatRes) => {
      ...
      filteredDetails.sort(
        (a, b) =>
          (b.userDetails.average_rating || 0) -
          (a.userDetails.average_rating || 0)
      );
...
const getJobPostDetails = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag FROM user_master WHERE user_id = ? AND delete_flag = 0";
    const values1 = [user_id];
    connection.query(query1, values1, async (err, result) => {
      if (err) {
        return response
      ...
      connection.query(query2, [job_post_id], async (err, jobPosts) => {
        ...
        const jobPostDetails = await Promise.all(
          jobPosts.map(async (job) => {
            ...
            const userImageQuery =
              "SELECT image FROM user_master WHERE user_id = ? AND delete_flag = 0";
            ...
            let jobdocument = [];
            const jobDocQuery =
              "SELECT file_name FROM job_file_master WHERE job_id = ? AND delete_flag = 0";
        ...
        if (check_hire_expert_id === 0) {
          querybid =
            "SELECT bid_id, expert_id, price, duration, files, createtime,status,duration_type, nda_file FROM bid_master WHERE job_post_id = ? AND delete_flag = 0";
          bidValues = [job_post_id];
        } else {
          querybid =
            "SELECT bid_id, expert_id, price, duration, files, createtime,status,duration_type, nda_file FROM bid_master WHERE job_post_id = ? AND delete_flag = 0 and expert_id=?";
        ...
        connection.query(querybid, bidValues, (err, bidResult) => {
          ...
          if (bidResult.length === 0) {
            return response
              ...
              .json({
                success: true,
                msg: languageMessage.dataFound,
                expertDetails: jobPostDetails,
                bidsDataArray: bidsDataArray,
                bidcount: bidResult.length,
              });
          }
          ...
          bidResult.forEach((bidItem) => {
            const expertQuery =
              "SELECT user_id, name, image, chat_charge, call_charge, video_call_charge, category FROM user_master WHERE delete_flag = 0 AND user_id = ?";
            connection.query(
              expertQuery,
              [bidItem.expert_id],
              async (err, expertResult) => {
                if (err) {
                  return response
                    .status(500)
                    .json({
                ...
                const categoryName =
                  "SELECT type_name, name FROM categories_master WHERE category_id = ? AND delete_flag= 0";
                connection.query(
                  categoryName,
                  [expertResult[0].category],
                  async (err, catResult) => {
                    if (err) {
                      return response
                        .status(200)
                    ...
                    const expertbidDetails =
                      expertResult.length > 0 ? expertResult[0] : null;
                    const expert_name = expertResult[0].name;
                    const status_label = "0=pending,1=hired";
                    const expert_image = expertResult[0].image
                      ? expertResult[0].image
                      : "NA";
...
const createJobPost = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const checkNda =
        "SELECT price FROM nda_price_master WHERE delete_flag = 0";
      connection.query(checkNda, async (infoErr, infoRes) => {
        ...
        if (nda_status == 1 && wallet_amount < nda_price) {
          return response
            .status(200)
            .json({ success: false, msg: languageMessage.NdaAmountErr });
        }
        ...
        connection.query(newUserQuery, values, async (err, result) => {
          if (err) {
            return response
              ...
              .json({
                success: false,
                msg: languageMessage.jobPostCreatedError,
                key: err,
              });
          }
          ...
          if (nda_status == 1) {
            const debitWallet =
              "INSERT INTO wallet_master (user_id, amount, status, type, createtime, updatetime) VALUES(?, ?, 1,2,?,?)";
            connection.query(
              debitWallet,
              [user_id, nda_price, now, now],
              async (walletErr, walletRes) => {
                if (walletErr) {
                  return response
                    .status(200)
                    .json({
          ...
          connection.query(
            ...
            (err, result1) => {
              ...
              return response
                ...
                .json({
                  success: true,
                  msg: languageMessage.jobPostCreated,
                  jobPostDataArray: result1[0],
                });
            }
          );
        });
...
const getExpertByFilter = async (request, response) => {
  const { user_id, state, city, category, sub_category, experience, rating } =
    request.body;
  try {
          ...
          if (rating) {
            ...
            finalResult = parsedResult
              .filter(
                (item) =>
                  item.userDetails &&
                  item.userDetails.average_rating == minRating
              )
              .map((item) => ({
      ...
      connection.query(query2, values2, (err, expertResult) => {
        ...
        const subCategoryLevelPromises = expertResult.map(async (expert) => {
          const subCategoryLevels = expert.sub_category_level
            .replace(/"/g, "")
            .split(",");
          const degrees = expert.degree.replace(/"/g, "").split(",");
          const languages = expert.language.replace(/"/g, "").split(",");
...
const walletRecharge = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag, wallet_balance FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type=1";
    const values1 = [user_id];
    connection.query(query1, values1, (err, result) => {
      if (err) {
        return response
      ...
      if (type == 0) {
        finalWallet =
          parseFloat(result[0].wallet_balance) + parseFloat(recharge_amount);
      } else if (type == 1) {
        finalWallet =
          parseFloat(result[0].wallet_balance) - parseFloat(recharge_amount);
      }
      ...
      connection.query(newUserQuery, values, async (err, result) => {
        if (err) {
          return response
            ...
            .json({
              success: false,
              msg: languageMessage.walletUpdateError,
              key: err,
            });
        }
        const newUserQuery = `
          ...
          async (err, result) => {
            ...
            if (result.affectedRows > 0) {
              ...
              return response
                ...
                .json({
                  success: true,
                  msg: languageMessage.walletUpdate,
...
const walletHistory = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const userDetails = await getUserDetails(user_id);
      const query2 =
        "SELECT transition_id, payment_transaction_id, expert_id, amount, createtime, status, type, user_id FROM wallet_master WHERE user_id = ? AND delete_flag = 0 ORDER BY createtime DESC";
      ...
      connection.query(query2, values2, async (err, walletresult) => {
        ...
        const finalBidResult = await Promise.all(
          walletresult.map(async (Item) => {
            ...
            return {
              ...
              posted_time: moment(Item.createtime)
                .add(5, "hours")
                .add(30, "minutes")
...
const getBidsOfJobPost = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const query2 =
        "SELECT bid_id, expert_id, price, duration, files, createtime FROM bid_master WHERE job_post_id = ? AND delete_flag = 0";
      connection.query(query2, [job_post_id], (err, bidResult) => {
        ...
        bidResult.forEach((bidItem) => {
          const expertQuery =
            "SELECT user_id, name, image FROM user_master WHERE delete_flag = 0 AND user_id = ?";
          connection.query(
            expertQuery,
            [bidItem.expert_id],
            (err, expertResult) => {
...
const hireTheExpert = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const checkJob =
        "SELECT job_post_id FROM job_post_master WHERE user_id = ? AND delete_flag = 0 AND job_post_id=?";
      ...
      connection.query(checkJob, jobvalues, (err, jobResult) => {
        if (err) {
          return response
            ...
            .json({
              success: false,
              msg: languageMessage.jobNotFound,
              key: err.message,
            });
        }
          ...
          async (err, result) => {
            if (err) {
              return response
                ...
                .json({
                  success: false,
                  msg: languageMessage.expertHireUnsuccess,
                  key: err,
                });
            }
            if (result.affectedRows > 0) {
              const checkJobbid =
                "SELECT bid_id FROM bid_master WHERE delete_flag = 0 AND job_post_id=? and expert_id=? and bid_id=?";
              ...
              connection.query(checkJobbid, jobbidvalues, (err, bidResult) => {
                if (err) {
                  return response
                    ...
                    .json({
                      success: false,
                      msg: languageMessage.jobBidNotFound,
                      key: err.message,
                    });
                }
                if (bidResult.length === 0) {
                    ...
                    if (resultbid.affectedRows > 0) {
                      ...
                      await getNotificationArrSingle(
                        ...
                        async (notification_arr_check) => {
                          let notification_arr_check_new = [
                          ...
                          if (
                            ...
                            const notiSendStatus =
                              await oneSignalNotificationSendCall(
                                notification_arr_check_new
                              );
                          } else {
                      ...
                      return response
                        ...
                        .json({
                          success: true,
                          msg: languageMessage.expertHireSuccess,
                        });
                    }
                  }
                );
      ...
      connection.query(
        ...
        async (err, result) => {
          ...
          if (result.affectedRows > 0) {
            return response
              .status(200)
              .json({ success: true, msg: languageMessage.projectCostCreate });
          }
        }
      );
    });
...
const getExpertEarning = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag FROM user_master WHERE user_id = ? AND delete_flag=0 AND user_type=2";
    const values1 = [user_id];
    connection.query(query1, values1, async (err, result) => {
      if (err) {
        return response
          ...
          connection.query(query2, [user_id], async (err, result) => {
            ...
            return response
              ...
              .json({
                success: true,
                msg: languageMessage.dataFound,
                total_expert_earning: total_expert_earning.toString(),
              });
          });
        });
      });
...
const withdrawRequest = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const sqlQuery =
        "SELECT mini_withdrawal_amt FROM commission_master WHERE delete_flag = 0";
          ...
          connection.query(query3, [user_id], async (err2, result2) => {
            ...
            if (withdraw_amount == minimum_amount) {
              ...
              connection.query(
                newUserQuery,
                values,
                async (err, requestresult) => {
                  if (err) {
                    return response
                      .status(200)
                      .json({
                        success: false,
                        msg: languageMessage.withdrawSendError,
                        key: err,
                      });
                  }
                  const userDetails = await getUserDetails(user_id);
                  return response
                    ...
                    .json({
                      success: true,
                      msg: languageMessage.withdrawSend,
              ...
              return response
                ...
                .json({
                  success: false,
                  msg: [
                    `${languageMessage.MinimumWithdrawalAmount} ₹ ${minimum_amount}`,
                  ],
                });
            }
            // }
...
const withdrawHistory = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const query1 =
        "SELECT expert_withdraw_id, expert_id, withdraw_status, withdraw_amount, total_earning_amount, withdraw_message, earning_after_withdraw,createtime, reason FROM expert_withdraw_master WHERE expert_id = ? order by expert_withdraw_id desc";
      const values1 = [user_id];
      connection.query(query1, values1, async (err, withdrawlresult) => {
        ...
        if (withdrawlresult.length === 0) {
          return response
            ...
            .json({
              success: true,
              msg: languageMessage.dataFound,
              withdrawRequestData: "NA",
            });
        }
        // Fetch user details for each item and category
        const finalBidResult = await Promise.all(
          withdrawlresult.map(async (Item) => {
            return {
              ...Item,
        ...
        return response
          ...
          .json({
            success: true,
            msg: languageMessage.dataFound,
            withdrawRequestData: finalBidResult,
          });
      });
    });
  } catch (err) {
        ...
        connection.query(query3, [usersIds], async (err, userResults) => {
          ...
          const combinedHistoryPosts = historyPosts.map((history) => {
            const user = userResults.find(
              (user) => user.user_id === history.user_id
            );
            return {
              ...history,
...
const getJobPostsForExpert = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag, category, sub_category FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type=2";
    const values1 = [user_id];
    connection.query(query1, values1, (err, result) => {
      if (err) {
        return response
...
const getExpertEarningHistory = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, result) => {
      ...
      const query1 =
        "SELECT expert_earning_id, total_amount, admin_commission_amount, commission_percentage, expert_earning, invoice_url, transition_id, createtime,type,user_id FROM expert_earning_master WHERE expert_id = ?  ORDER BY createtime DESC";
      const values1 = [user_id];
          ...
          connection.query(query2, [user_id], async (err3, result3) => {
            ...
            return response
              ...
              .json({
                ...
                msg: languageMessage.dataFound,
                total_expert_earning: total_expert_earning.toString(),
                earning_history: finalBidResult,
              });
          });
        });
      });
      ...
      connection.query(query2, [user_id], (err, historyPosts) => {
        ...
        connection.query(query3, [usersIds], async (err, userResults) => {
          ...
          const get_avg_rating =
            total_review_count > 0 ? avg_rating / total_review_count : 0;
          return response
            .status(200)
            .json({
              success: true,
              ...
              rating_bar,
              total_review_count,
              avg_rating: get_avg_rating.toFixed(1),
              expert_image,
            });
        });
      });
            ...
            async (err, buyresult) => {
              if (err) {
                return response
                  ...
                  .json({
                    success: false,
                    msg: languageMessage.buySubscriptionUnsuccess,
                    key: err,
                  });
              }
              if (buyresult.affectedRows > 0) {
                return response
                  ...
                  .json({
                    success: true,
                    msg: languageMessage.buySubscriptionSuccess,
                  });
              } else {
                return response
                  .status(200)
      ...
      connection.query(newUserQuery, values, async (err, result) => {
        if (err) {
          return response
            ...
            .json({
              success: false,
              msg: languageMessage.replySentUnsuccess,
              key: err,
            });
        }
        return response
          .status(200)
          .json({ success: true, msg: languageMessage.replySentSuccess });
      });
    });
  } catch (err) {
    return response
...
const rateExpert = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag,name FROM user_master WHERE user_id = ? AND delete_flag = 0";
    const values1 = [user_id];
    connection.query(query1, values1, async (err, result) => {
      if (err) {
        return response
      ...
      connection.query(newUserQuery, values, async (err, ratingresult) => {
        if (err) {
          return response
            ...
            .json({
              success: false,
              msg: languageMessage.ratingSentUnsuccess,
              key: err,
            });
        }
        const user_id_notification = user_id;
        ...
        return response
          .status(200)
          .json({ success: true, msg: languageMessage.ratingSentSuccess });
      });
    });
  } catch (err) {
    return response
    ...
    connection.query(userQuery, [user_id], (err, userResult) => {
      ...
      connection.query(callHistoryQuery, [user_id], (err, callHistory) => {
        ...
        callHistory.forEach((record) => {
          const expertDetailQuery =
            "SELECT name, image, category,call_charge,video_call_charge,chat_charge FROM user_master WHERE user_id = ? AND delete_flag = 0";
          connection.query(
            expertDetailQuery,
            ...
            (err, expertResult) => {
              ...
              const expertDetails =
                expertResult.length > 0
                  ? expertResult[0]
                  : { name: "NA", image: "NA", category: "NA" };
              // Fetch category name
              const categoryQuery =
                "SELECT name FROM categories_master WHERE category_id = ?";
              connection.query(
                categoryQuery,
                ...
                (err, categoryResult) => {
                  ...
                  const categoryName =
                    categoryResult.length > 0 ? categoryResult[0].name : "NA";
...
const ExpertCallHistory = async (request, response) => {
  ...
  try {
    // Check if the user exists
    const userQuery =
      "SELECT mobile, active_flag,category FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type = 2";
    connection.query(userQuery, [user_id], (err, userResult) => {
      if (err) {
        return response
          .status(500)
      ...
      connection.query(callHistoryQuery, [user_id], (err, callHistory) => {
        ...
        callHistory.forEach((record) => {
          const expertDetailQuery =
            "SELECT name, image FROM user_master WHERE user_id = ? AND delete_flag = 0";
          connection.query(
            expertDetailQuery,
            [record.user_id],
            (err, expertResult) => {
...
const ExpertBidJob = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const checkJob =
        "SELECT user_id FROM job_post_master WHERE job_post_id = ? AND delete_flag = 0";
        ...
        connection.query(bidQuery, values, async (err, bidresult) => {
          if (err) {
            return response
              ...
              .json({
                success: false,
                msg: languageMessage.bidSentUnsuccess,
                key: err,
              });
          }
          const user_id_notification = user_id;
          ...
          return response
            .status(200)
            .json({ success: true, msg: languageMessage.bidSentSuccess });
        });
      });
    });
  } catch (err) {
...
const getExpertHomeJobs = (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag, expert_status FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type=2";
    const values1 = [user_id];
    connection.query(query1, values1, async (err, result) => {
      if (err) {
        return response
      ...
      if (result[0].expert_status === 0) {
        return response
          ...
          .json({
            success: false,
            msg: languageMessage.ExpertRequestNotApproved,
          });
      }
      ...
      if (result[0].expert_status === 2) {
        return response
          .status(200)
          .json({ success: false, msg: languageMessage.ExpertRejected });
      }
      ...
      connection.query(checkSubscription, [user_id], async (subErr, subRes) => {
        ...
        if (subRes.length > 0) {
          ...
          if (now > expiryDate) {
            return response
              ...
              .json({
                success: false,
                msg: languageMessage.SubscriptionExpired,
              });
          }
        }
        ...
        return response
          ...
          .json({
            ...
            userjobPostCount: userjobPostCount,
            completedJobDetails: completedJobDetails,
          });
      });
    });
  } catch (err) {
...
const bookMarkJob = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT mobile, active_flag, wallet_balance FROM user_master WHERE user_id = ? AND delete_flag = 0";
    const values1 = [user_id];
    connection.query(query1, values1, (err, userResult) => {
      if (err) {
        return response
      ...
      const checkJob =
        "SELECT job_post_id FROM job_post_master WHERE delete_flag = 0 AND job_post_id=?";
            ...
            async (err, result) => {
              if (err) {
                return response
                  ...
                  .json({
                    success: false,
                    msg: languageMessage.bookMarkUnsuccess,
                    key: err,
                  });
              }
              return response
                .status(200)
                .json({ success: true, msg: languageMessage.bookMarkSuccess });
            }
          );
        } else {
            ...
            async (err, result) => {
              if (err) {
                return response
                  ...
                  .json({
                    success: false,
                    msg: languageMessage.jobUnsaveUnsuccess,
                    key: err,
                  });
              }
              return response
                .status(200)
                .json({ success: true, msg: languageMessage.jobSaveSuccess });
            }
          );
        }
      });
          ...
          async (err, result) => {
            if (err) {
              return response
                ...
                .json({
                  success: false,
                  msg: languageMessage.jobReportUnsuccess,
                  key: err,
                });
            }
            return response
              .status(200)
              .json({ success: true, msg: languageMessage.jobReportSuccess });
          }
        );
      });
    });
...
const customerJobFilter = (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const userjobPost = await getCustomerJobFilter(
        user_id,
        category,
        sub_category
      );
...
const expertJobFilter = (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const userjobPost = await getExpertJobFilter(
        user_id,
        category,
        sub_category
      );
...
const createJobCost = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const checkJob =
        "SELECT job_post_id FROM job_post_master WHERE delete_flag = 0 AND job_post_id=? and user_id = ? and assign_expert_id!=0";
      ...
      connection.query(checkJob, jobvalues, (err, jobResult) => {
        ...
        connection.query(
          bookMarkQuery,
          [project_cost, job_post_id, user_id],
          async (err, result) => {
            if (err) {
              return response
                .status(200)
                .json({
                  success: false,
                  msg: languageMessage.projectCostUnsuccess,
                  key: err,
                });
            }
            return response
              .status(200)
              .json({ success: true, msg: languageMessage.projectCostSuccess });
          }
        );
      });
    });
...
const createJobMilestone = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT name,mobile, active_flag, wallet_balance FROM user_master WHERE user_id = ? AND delete_flag = 0";
    const values1 = [user_id];
    connection.query(query1, values1, async (err, userResult) => {
      ...
      const checkJob =
        "SELECT job_post_id,title,assign_expert_id FROM job_post_master WHERE delete_flag = 0 AND job_post_id=? and user_id = ? and assign_expert_id!=0";
      const jobvalues = [job_post_id, user_id];
      connection.query(checkJob, jobvalues, async (err, jobResult) => {
        ...
        if (wallet_amount < amount) {
          return response
            ...
            .json({
              success: false,
              msg: languageMessage.WalletbalanceInvalid,
            });
        }
          ...
          async (err, result) => {
            if (err) {
              return response
                ...
                .json({
                  success: false,
                  msg: languageMessage.milestoneCreateUnsuccess,
                  key: err,
                });
            }
            const user_id_notification = user_id;
            ...
            return response
              ...
              .json({
                success: true,
                msg: languageMessage.milestoneCreateSuccess,
              });
          }
        );
      });
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const checkMilestone =
        "SELECT job_post_id FROM milestone_master WHERE delete_flag = 0 AND milestone_id=? and milestone_status=0";
        ...
        (err, milestoneResult) => {
          if (err) {
            return response
              ...
              .json({
                success: false,
                msg: languageMessage.WorkSpaceNotFound,
                key: err.message,
              });
          }
          if (milestoneResult.length === 0) {
            return response
              .status(404)
              .json({ success: false, msg: languageMessage.WorkSpaceNotFound });
          }
          const checkJob =
            "SELECT user_id,title FROM job_post_master WHERE delete_flag = 0 AND job_post_id=? and assign_expert_id=?";
              ...
              async (err, updateResult) => {
                if (err) {
                  if (type == 1) {
                    return response
                      ...
                      .json({
                        success: false,
                        msg: languageMessage.WorkAcceptUnsuccess,
                        key: err,
                      });
                  } else {
                    return response
                      ...
                      .json({
                        success: false,
                        msg: languageMessage.WorkRejectUnsuccess,
                        key: err,
                      });
                  }
                }
                ...
                if (type == 1) {
                  return response
                    ...
                    .json({
                      success: true,
                      msg: languageMessage.WorkAcceptSuccess,
                    });
                } else {
                  return response
                    .status(200)
                    .json({
                      success: true,
                      msg: languageMessage.WorkRejectSuccess,
                    });
                }
              }
            );
...
const sentMilestoneRequest = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const user_name = userResult[0].name;
      const checkMilestone =
        "SELECT job_post_id FROM milestone_master WHERE delete_flag = 0 AND milestone_id=? and milestone_status=7";
      const milestonevalues = [milestone_id];
      connection.query(
        checkMilestone,
        milestonevalues,
              ...
              async (err, updateResult) => {
                if (err) {
                  return response
                    ...
                    .json({
                      success: false,
                      msg: languageMessage.milestoneRequestUnsuccess,
                      key: err,
                    });
                }
                const user_id_notification = user_id;
                ...
                return response
                  ...
                  .json({
                    success: true,
                    msg: languageMessage.milestoneRequestSuccess,
                  });
              }
            );
          });
...
const checkMilestoneRequest = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const user_name = userResult[0].name;
      const checkMilestone =
        "SELECT job_post_id FROM milestone_master WHERE delete_flag = 0 AND milestone_id=? and milestone_status=3";
      const milestonevalues = [milestone_id];
      connection.query(
        checkMilestone,
        milestonevalues,
        (err, milestoneResult) => {
          ...
          const checkJob =
            "SELECT assign_expert_id,title FROM job_post_master WHERE delete_flag = 0 AND job_post_id=? and user_id=?";
          const jobvalues = [milestoneResult[0].job_post_id, user_id];
          connection.query(checkJob, jobvalues, (err, jobResult) => {
            if (err) {
              return response
            ...
            const sql =
              "SELECT price FROM milestone_master WHERE milestone_id = ? AND delete_flag = 0";
              ...
              connection.query(
                ...
                async (err, updateResult) => {
                  if (err) {
                    if (type == 4) {
                      return response
                        ...
                        .json({
                          success: false,
                          msg: languageMessage.milestoneReleaseUnsuccess,
                          key: err.message,
                        });
                    }
                    if (type == 5) {
                      return response
                        ...
                        .json({
                          success: false,
                          msg: languageMessage.milestoneDisputeUnsuccess,
                          key: err,
                        });
                    }
                    if (type == 6) {
                      return response
                        ...
                        .json({
                          success: false,
                          msg: languageMessage.milestoneCancelUnsuccess,
                          key: err,
                        });
                    }
                  }
                  ...
                  if (type == 4) {
                    ...
                    connection.query(
                      fileInsertQuery,
                      [user_id, expert_id, milestone_price],
                      async (err, result1) => {
                        if (err) {
                          return response
                            .status(200)
                        ...
                        var expert_earning = await getExpertEarningg(
                          milestone_id,
                          user_id
                        );
                        return response
                          ...
                          .json({
                            success: true,
                            msg: languageMessage.milestoneReleaseSuccess,
                            expert_earning_id: expert_earning,
                          });
                      }
                    );
                  }
                  if (type == 5) {
                    return response
                      ...
                      .json({
                        success: true,
                        msg: languageMessage.milestoneDisputeSuccess,
                      });
                  }
                  if (type == 6) {
                    return response
                      ...
                      .json({
                        success: true,
                        msg: languageMessage.milestoneCancelSuccess,
                      });
                  }
                }
              );
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      connection.query(query2, [job_post_id], async (err, jobPosts) => {
        ...
        querybid =
          "SELECT bid_id, expert_id, price, duration, files, createtime,status,duration_type FROM bid_master WHERE job_post_id = ? AND delete_flag = 0 and expert_id=?";
        ...
        try {
          ...
          if (rating) {
            finalResult = parsedResult.filter(
              (item) =>
                item.average_rating >= parseFloat(rating) &&
                item.average_rating < parseFloat(rating) + 1
            );
          }
          if (finalResult.length === 0) {
            return response
...
const logOut = (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const delete_query =
        "DELETE FROM `user_notification` WHERE user_id = ? AND delete_flag = 0";
      const delete_values = [user_id];
      connection.query(
        delete_query,
        delete_values,
        async (err, delete_result) => {
          return response
            .status(200)
            .json({ success: true, msg: languageMessage.logOutSuccess });
        }
      );
    });
  } catch (err) {
...
const chatFileUpload = async (request, response) => {
  ...
  if (request.files && request.files["image"]) {
    let files = Array.isArray(request.files["image"])
      ? request.files["image"]
      : [request.files["image"]];
    ...
    try {
      ...
      return response
        .status(200)
        .json({ success: true, msg: languageMessage.fileUploadedSuccess });
    } catch (err) {
      return response
        .status(500)
        .json({
          success: false,
          msg: languageMessage.fileUploadedError,
          key: err.message,
        });
    }
  } else {
    return response
      .status(400)
      .json({ success: false, msg: "No files uploaded", key: "image" });
  }
};
//end
    ...
    Promise.all(queries)
      ...
      .then(async () => {
        try {
          ...
          res
            .status(200)
            .json({ success: true, msg: languageMessage.AvailibilityCreated });
        } catch (err) {
  ...
  connection.query(query1, values1, async (err, result) => {
          ...
          if (results.length > 0) {
            ...
            connection.query(
              ...
              (updateErr) => {
                ...
                if (currentStatus === 0) {
                  clearAndInsertSlots(availabilityId, req.body, day)
                    .then(resolve)
                    .catch(reject);
                } else {
                  clearSlots(availabilityId).then(resolve).catch(reject);
    ...
    Promise.all(updatePromises)
      .then(() =>
        res
          .status(200)
          .json({ success: true, msg: languageMessage.availabilityUpdated })
      )
      .catch((error) => {
        console.error("Error updating availability:", error);
        res
      ...
      connection.query(getAvailableSlotsQuery, [user_id], (err, slots) => {
        ...
        const available_slots = slots.reduce((acc, slot) => {
          ...
          const formatTimeToAMPM = (time24) => {
            ...
            const timeStr =
              typeof time24 === "string" ? time24 : time24.toISOString();
...
const userBookSlot = async (request, response) => {
  let { user_id, expert_id, availability_id, slot_id, day, date, type } =
    request.body;
  ...
  try {
    const query =
      "SELECT mobile, name, active_flag FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type=1";
    ...
    connection.query(query, values, async (err, result) => {
      ...
      const availabilityquery =
        "SELECT status FROM availability_master WHERE user_id = ? AND delete_flag = 0 AND availability_id=?";
      const availabilityvalues = [expert_id, availability_id];
      connection.query(
        availabilityquery,
        availabilityvalues,
        async (err, availabilityresult) => {
          if (err) {
            return response
              .status(200)
              .json({
          ...
          if (availabilityresult.length === 0) {
            return response
              ...
              .json({
                success: false,
                msg: languageMessage.AvailibilityNotFound,
              });
          }
          //----------------------check slot---------------------------
          const slotquery =
            "SELECT slot_id FROM slot_master WHERE slot_id = ? AND delete_flag = 0 AND availability_id=?";
          ...
          connection.query(slotquery, slotvalues, async (err, slotresult) => {
            ...
            if (slotresult.length === 0) {
              return response
                .status(200)
                .json({ success: false, msg: languageMessage.SlotNotFound });
            }
            //-------------------book slot----------------------------
            ...
            connection.query(bookQuery, bookvalues, async (err, bookresult) => {
              if (err) {
                return response
                  ...
                  .json({
                    success: false,
                    msg: languageMessage.Slotbookerror,
                    key: err,
                  });
              }
              ...
              return response
                .status(200)
                .json({ success: true, msg: languageMessage.CallScheduled });
            });
          });
        }
      );
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      connection.query(
        ...
        (err, slots) => {
          ...
          slots.map((item) => {
            item.start_time = moment(item.start_time, "HH:mm:ss").format(
              "hh:mm A"
            );
...
const convertIntoMilestone = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, (err, userResult) => {
      ...
      const user_name = userResult[0].name;
      const checkMilestone =
        "SELECT job_post_id FROM milestone_master WHERE delete_flag = 0 AND milestone_id=? and milestone_status=1";
      const milestonevalues = [milestone_id];
      connection.query(
        checkMilestone,
        milestonevalues,
              ...
              async (err, updateResult) => {
                if (err) {
                  return response
                    ...
                    .json({
                      success: false,
                      msg: languageMessage.convertMilestoneError,
                      key: err,
                    });
                }
                const user_id_notification = user_id;
                ...
                return response
                  ...
                  .json({
                    success: true,
                    msg: languageMessage.convertMilestoneSuccess,
                  });
              }
            );
          });
...
const updateJobMilestone = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT name,active_flag FROM user_master WHERE user_id = ? AND delete_flag = 0";
    const values1 = [user_id];
    connection.query(query1, values1, (err, userResult) => {
      if (err) {
        return response
      ...
      const checkJob =
        "SELECT job_post_id FROM milestone_master WHERE delete_flag = 0 AND milestone_id=?";
          ...
          async (err, result) => {
            if (err) {
              return response
                ...
                .json({
                  success: false,
                  msg: languageMessage.milestoneUpdatedUnsuccess,
                  key: err,
                });
            }
            return response
              ...
              .json({
                success: true,
                msg: languageMessage.milestoneUpdatedSuccess,
              });
          }
        );
      });
...
const checkWalletAmount = async (request, response) => {
  ...
  try {
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      const query2 =
        "SELECT call_charge, video_call_charge, chat_charge FROM user_master WHERE user_id = ? AND delete_flag = 0 AND user_type=2";
          ...
          if (checkResult.length > 0) {
            ...
            connection.query(
              ...
              (err, updateResult) => {
                ...
                return response
                  ...
                  .json({
                    success: true,
                    msg: languageMessage.walletDebitUpdate,
                  });
              }
            );
          } else {
            ...
            connection.query(
              ...
              async (err, insertResult) => {
                ...
                const updatebalance =
                  "UPDATE wallet_master SET wallet_balance = ?, updatetime = NOW() WHERE transition_id = ? AND delete_flag = 0";
                connection.query(
                  updatebalance,
                  [userWalletbalance, transition_id],
                  async (updateErr, updateRes) => {
                    if (updateErr) {
                      return response
                        .status(200)
                        .json({
...
const getTokenVariable = async (request, response) => {
  try {
    const sql =
      "SELECT token, variable FROM token_variable_master WHERE delete_flag = 0";
    connection.query(sql, async (err, res) => {
      ...
      return response
        .status(200)
        .json({ success: true, msg: languageMessage.dataFound, data: data });
    });
  } catch (error) {
    return response
      .status(200)
...
async function getExpertEarningg(milestone_id, user_id) {
  ...
  `;
    connection.query(sql, [milestone_id], async (err, result) => {
      ...
      const sql1 =
        "SELECT gst, tds, tcs, platform_fee, commission_percentage FROM commission_master WHERE delete_flag = 0";
      connection.query(sql1, async (err1, res1) => {
        ...
        let admin_commission_amount = parseFloat(
          ((received_amount * commission_percentage) / 100).toFixed(2)
        );
        let expert_earning = parseFloat(
          (received_amount - admin_commission_amount).toFixed(2)
        );
        ...
        if (info.gst_number > 0) {
          gst_amount = parseFloat(
            ((received_amount * gst) / (100 + gst)).toFixed(2)
          );
          net_amount = parseFloat((received_amount - gst_amount).toFixed(2));

          platform_fee_amount = parseFloat(
            ((net_amount * platform_fee) / 100).toFixed(2)
          );
          platform_fee_gst_amount = parseFloat(
            ((platform_fee_amount * gst) / 100).toFixed(2)
          );

          tds_amount = parseFloat(((net_amount * tds) / 100).toFixed(2));
          tcs_amount = parseFloat(((net_amount * tcs) / 100).toFixed(2));

          grand_total_earning = parseFloat(
            (
              platform_fee_amount +
              platform_fee_gst_amount -
          ...
          let apply_gst_amount = parseFloat(
            ((platform_fee_amount * gst) / 100).toFixed(2)
          );
          let net_apply_gst_amount = apply_gst_amount / 2;
          grand_total_earning = parseFloat(
            (platform_fee_amount - net_apply_gst_amount).toFixed(2)
          );
          const now = moment().tz("Asia/Kolkata").format("YYYY-MM-DD HH:mm:ss");
...
const completeJob = async (request, response) => {
  ...
  try {
    ...
    const checkUser =
      "SELECT user_id, active_flag, email, name FROM user_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(checkUser, [user_id], async (err, res) => {
      if (err) {
        return response
          .status(200)
      ...
      const jobSql =
        "SELECT title FROM job_post_master WHERE job_post_id = ? AND delete_flag = 0";
      connection.query(jobSql, [job_post_id], async (jobErr, jobRes) => {
        ...
        if (jobRes.length == 0) {
          return response
            .status(200)
            .json({ sucesss: false, msg: languageMessage.jobNotFound });
        }

        let job_name = jobRes[0].title;

        const sql =
          "UPDATE job_post_master SET status = 3, updatetime = NOW() WHERE job_post_id = ? AND delete_flag = 0";
        connection.query(sql, [job_post_id], async (err1, res1) => {
          ...
          if (res1.affectedRows == 0) {
            return response
              .status(200)
              .json({ success: false, msg: languageMessage.updateErr });
          }
          ...
          return response
            .status(200)
            .json({ success: true, msg: languageMessage.JobCompleted });
          // }
          // });
        });
      });
  ...
  try {
    ...
    connection.query(sql, [expert_earning_id], async (err, res) => {
      ...
      try {
        ...
        const invoiceUrl = filename;
        const updateSql =
          "UPDATE expert_earning_master SET invoice_url = ? WHERE expert_earning_id = ?";
        connection.query(
          updateSql,
          [invoiceUrl, expert_earning_id],
          (updateErr) => {
            if (updateErr) {
              return response
                .status(200)
                .json({
                  success: false,
                  msg: languageMessage.ErrorUpdatingUrl,
                  error: updateErr.message,
                });
            }
            return response
              ...
              .json({
                success: true,
                msg: languageMessage.PdfGeneratedSuccess,
        ...
        return response
          ...
          .json({
            success: false,
            msg: languageMessage.ErrorGeneratingPdf,
      ...
      const htmlContent = `
            ...
            <body>
              <div class="invoice-container">
                ...
                <p>This is the receipt for a payment of <strong>₹${
                  invoiceData.grand_total_expert_earning
                }</strong> you made to milestone.</p>
                ...
                <div class="section">
                  <div class="info-row">
                    <div class="info-block">
                      ...
                      <div><strong>${
                        invoiceData.milestone_number
                      }</strong></div>
                    </div>
                    <div class="info-block">
                      <div class="text-muted">Payment Date</div>
                      <div><strong>${moment(invoiceData.createtime).format(
                        "MMM DD, YYYY"
                      )}</strong></div>
                    </div>
                  </div>
                ...
                <div class="section">
                  <div class="info-row">
                    <div class="info-block">
                      ...
                      <div><a href="mailto:${invoiceData.user_email}">${
        invoiceData.user_email
                    ...
                    <div class="info-block">
                      ...
                      <div>${invoiceData.address}, ${
        invoiceData.city_name
      }</div>
                      <div><a href="mailto:${invoiceData.email}">${
        invoiceData.email
      }</a></div>
                    </div>
                  </div>
...
const getWalletPdf = async (request, response) => {
  ...
  try {
    ...
    const sql =
      "SELECT wm.user_id, wm.expert_id, wm.amount, wm.wallet_balance, wm.status, wm.type, um.name, wm.createtime, wm.payment_transaction_id, um.email, um.address FROM wallet_master wm JOIN user_master um ON wm.user_id = um.user_id WHERE wm.transition_id = ? AND wm.delete_flag = 0";
    connection.query(sql, [transition_id], async (err, res) => {
      if (err) {
        return response
          .status(200)
      ...
      if (res.length == 0) {
        return response
          .status(200)
          .json({ success: false, msg: languageMessage.dataFound });
      }
      try {
        let type_label =
          res[0].type === 1
        ...
        const invoiceUrl = filename;
        const updateSql =
          "UPDATE wallet_master SET invoice_url = ? WHERE transition_id = ?";
        connection.query(
          updateSql,
          [invoiceUrl, transition_id],
          (updateErr) => {
            if (updateErr) {
              return response
                .status(200)
...
<body>
  <div class="invoice-container">
    ...
    <p>This is the receipt for a payment of <strong>₹${
      invoiceData.amount
    }</strong> you made to ${type_label}.</p>
...
const getExpertAllEarningPdf = async (request, response) => {
  ...
  try {
    ...
    const sql =
      "SELECT em.type,  em.expert_earning, em.createtime, um.name FROM expert_earning_master em JOIN user_master um ON em.expert_id = um.user_id WHERE em.expert_earning_id = ? AND em.delete_flag = 0";
    connection.query(sql, [expert_earning_id], async (err, res) => {
      if (err) {
        return response
          .status(200)
...
const getCustomerMilestoneCharge = async (request, response) => {
  ...
  try {
    ...
    const sql =
      "SELECT em.grand_total_expert_earning, em.createtime, em.milestone_id, mm.milestone_number, em.expert_id, em.user_id AS customer_id, um.name, umm.name AS cust_name,  um.email, um.address, um.city FROM expert_earning_master em JOIN milestone_master mm ON em.milestone_id = mm.milestone_id JOIN user_master um ON em.expert_id = um.user_id JOIN city_master cm ON um.city = cm.city_id JOIN user_master umm ON em.user_id = umm.user_id WHERE em.milestone_id = ? AND em.delete_flag = 0";
    connection.query(sql, [milestone_id], async (err, res) => {
      if (err) {
        return response
          .status(200)
...
<body>
  <div class="invoice-container">
    ...
    <div class="header">
      ...
      <p>This is the receipt for a payment of <strong>₹${
        invoiceData.grand_total_expert_earning
      }</strong> you paid to ${invoiceData.name}.</p>
    </div>
    <div class="section">
      <div class="info-row">
        <div class="info-block">
...
const getNdaPrice = async (request, response) => {
  ...
  try {
    ...
    connection.query(checkUser, [user_id], async (err, res) => {
      ...
      const sql =
        "SELECT price, image FROM nda_price_master WHERE delete_flag = 0";
      connection.query(sql, async (err1, res1) => {
        ...
        return response.status(200).json({
          success: true,
          msg: languageMessage.dataFound,
          nda_price: res1[0].price.toString(),
          image: res1[0].image,
        });
      });
    });
...
const userChatStatus = async (request, response) => {
  ...
  try {
    ...
    const checkUserQuery =
      "SELECT user_id, name, active_flag FROM user_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(checkUserQuery, [user_id], async (err, userRes) => {
      ...
      connection.query(checkOtherUser, [other_user_id], async (err1, res1) => {
        ...
        const check =
          "SELECT * FROM chat_status_master WHERE user_id = ? AND delete_flag = 0";
        connection.query(check, [user_id], async (err2, res2) => {
          ...
          if (res2.length > 0) {
            const update =
              "UPDATE chat_status_master SET other_user_id = ?, updatetime = NOW() WHERE user_id = ? AND delete_flag= 0";
            connection.query(
              update,
              ...
              async (updateErr, updateRes) => {
                ...
                if (updateRes.affectedRows > 0) {
                  return response
                    .status(200)
                    .json({ success: true, msg: languageMessage.dataFound });
                }
              }
            );
          } else {
            const insert =
              "INSERT INTO chat_status_master (user_id, other_user_id, createtime, updatetime) VALUES(?, ?, NOW(), NOW())";
            connection.query(
              insert,
  ...
  try {
    ...
    connection.query(checkUserQuery, [user_id], async (err, userRes) => {
      ...
      connection.query(checkOtherUser, [other_user_id], async (err1, res1) => {
        ...
        const sql =
          "SELECT chat_status_id FROM chat_status_master WHERE other_user_id =? AND user_id = ? AND delete_flag = 0";
...
const initiatePayment = (req, res) => {
  ...
  const checkUser =
    "SELECT email, mobile, name, active_flag FROM user_master WHERE user_id = ? AND delete_flag = 0";
  connection.query(checkUser, [user_id], async (err, result) => {
    ...
    const furl =
      "https://zqd422dn6n.ap-south-1.awsapprunner.com/payment_failure";
...
const getSubscriptionStatus = async (request, response) => {
  ...
  try {
    ...
    const checkUser =
      "SELECT user_id, active_flag, expert_status, user_type FROM user_master WHERE user_id = ? AND delete_flag = 0";
    connection.query(checkUser, [user_id], (err, userRes) => {
      ...
      if (userRes[0].user_type == 2) {
        if (userRes[0].expert_status == 0) {
          return response
            .status(200)
            .json({ success: false, msg: languageMessage.accountunderReview });
        }
      }
...
const paymentHideShow = async (request, response) => {
  try {
    const sql =
      "SELECT status FROM payment_master WHERE payment_id = 1 AND  delete_flag = 0";
    connection.query(sql, async (err, res) => {
      if (err) {
        return response
          .status(200)
...
const refundRequest = async (request, response) => {
  const { user_id, name, email, mobile, request_title, description, amount } =
    request.body;
  try {
    if (!user_id)
      return response
    ...
    connection.query(checkUser, [user_id], async (err, userRes) => {
      try {
        ...
        const sql =
          "INSERT INTO refund_request_master(user_id, unique_no, name, email,mobile, description, title, otp, refund_amount, createtime, updatetime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
        connection.query(
          sql,
          [
            user_id,
              ...
              if (res1.affectedRows > 0) {
                ...
                return response
                  ...
                  .json({
                    ...
                    refund_id: refund_id,
                    refund_request_data: refund_request_data,
                  });
                ...
                return response
                  ...
                  .json({
                    success: false,
                    msg: languageMessage.RefundRequestNotSent,
                  });
              }
            } catch (e1) {
              return response
...
const refundOtpVerify = async (request, response) => {
  ...
  try {
    const query1 =
      "SELECT * FROM refund_request_master WHERE refund_id = ? AND delete_flag= 0";
    ...
    connection.query(query1, values1, async (err, result) => {
      ...
      if (refundOtp !== otp) {
        return response
          .status(200)
          .json({ success: false, msg: languageMessage.invalidOtp });
      }
      ...
      connection.query(verifyOtp, [refund_id], async (err) => {
        ...
        return response
          ...
          .json({
            success: true,
            msg: languageMessage.otpVerifiedSuccess,
            new_data: new_data,
          });
      });
    });
...
const getRefundStatus = async (request, response) => {
  ...
  try {
    ...
    connection.query(checkUser, [user_id], async (err, userRes) => {
      ...
      const check =
        "SELECT refund_id, unique_no, title, description, refund_status, transaction_id , refund_amount, createtime FROM refund_request_master WHERE user_id = ? AND delete_flag= 0 AND otp_verify= 1 ORDER BY createtime DESC";
  ...
  try {
    ...
    connection.query(getScheduleQuery, today, async (err, res) => {
      ...
      for (let data of res) {
        ...
        const oneHourBefore = scheduledDateTime
          .clone()
          .subtract(1, "hours")
        ...
        if (oneHourBefore === currentTime) {
          ...
          const action =
            data.type == 1 ? "video_call_reminder" : "voice_call_reminder";
          const title =
            data.type == 1 ? "Video Call Reminder" : "Voice Call Reminder";
          const messages =
            data.type == 1
      ...
      return response
        .status(200)
        .json({ success: true, msg: "Notifications processed" });
    });
  } catch (error) {
...
const manageChatCharge = async (request, response) => {
  ...
  try {
    ...
    connection.query(checkUser, [user_id], async (err, res) => {
      ...
      const sql =
        "INSERT INTO video_call_master(user_id, other_user_id, type, price, createtime, updatetime) VALUES(?, ?, 3, ?, NOW(), NOW())";
      connection.query(
        sql,
        [user_id, other_user_id, amount],
        async (err1, res1) => {
          if (err1) {
            return response
              .status(200)
          ...
          connection.query(
            insertQuery,
            [user_id, other_user_id, amount, 1, 3, video_call_id, now, now],
            async (err, insertResult) => {
              if (err)
                return response.status(200).json({
                  success: false,
          ...
          const expert_earning = await getExpertCallEarning(
            user_id,
            other_user_id,
            amount,
            video_call_id
          );
          const { admin_final_earning, grand_total_earning } =
            expert_earning[0];
          ...
          connection.query(
            update,
            [admin_final_earning, grand_total_earning, video_call_id],
            async (updateErr, updateRes) => {
              if (updateErr) {
                return response
                  .status(200)
          ...
          return response
            .status(200)
            .json({ success: true, msg: languageMessage.DetailsAdded });
        });
    });   });
            });
...
async function getExpertCallEarning(
  customer_id,
  expert_id,
  call_Charges,
  video_call_id
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql, [expert_id], async (err, result) => {
      ...
      let gst_number =
        result.length > 0 && result[0].gst_number ? result[0].gst_number : null;

      const sql1 =
        "SELECT gst, tds, tcs, platform_fee FROM commission_master WHERE delete_flag = 0";
      connection.query(sql1, async (err1, res1) => {
        if (err1) {
          return reject(err1);
        }
